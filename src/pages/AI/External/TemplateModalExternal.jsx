...
import css from './TemplateModalExternal.module.css';
import { v4 as uuidv4 } from 'uuid';
import { createSetting, getSettingByType, updateSetting } from '../../../apis/settingService.jsx';

const TEMPLATE_SETTING_TYPE = 'AI_TEMPLATE_QUESTIONS_EXTERNAL';

export default function TemplateModalExternal({ isOpen, onClose, onSelectTemplate, currentUser }) {
    ...
    const handleEditTemplate = async (template) => {
        if (!currentUser?.isAdmin) {
            message.error('Chỉ quản trị viên mới có thể sửa mẫu câu hỏi');
            return;
        }
        ...
        try {
            const updatedTemplates = templates.map(t => 
                t.id === template.id 
                    ? { ...t, question: editingTemplate.question }
                    : t
            );
