...
const { Option } = Select;
const calculationOptions = ['Tổng',
	 'Trung bình', 
	//  'Trung bình có trọng số'
	];

export function SettingLuyKeDL({ isOpen, setIsOpen, dataDoLuong, dataKinhDoanh, khkdTH, onSuccess }) {
	// Khởi tạo settings mặc định mỗi name là 'Tổng'
	const [settings, setSettings] = useState({});

	useEffect(() => {
		if (isOpen) {
			const existedSettings = khkdTH?.luyKeDL || {};
	...
	return (
		<Modal
			...
			open={isOpen}
			onOk={() => {
				updateKHKDTongHop({...khkdTH, luyKeDL: settings}).then(()=> {
					setIsOpen(false);
					if (onSuccess) {
						onSuccess();
					}
