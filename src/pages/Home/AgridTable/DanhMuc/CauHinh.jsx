...
import {createNewCauHinh, getAllCauHinh} from "../../../../apis/cauHinhService.jsx";
import {DeleteFilterIcon, EllipsisIcon, FilterIcon} from "../../../../icon/IconSVG.js";
...
import {getAllCompany, getCompanyDataById} from "../../../../apis/companyService.jsx";
...
export default function CauHinh({isShowCauHinh}) {
    const table = 'CauHinh';
    const tableCol = 'CauHinhCol';
    const tableFilter = 'CauHinhFilter';
    ...
    const loadData = async () => {
        const data = await getAllCauHinh()
        const ctys = await getAllCompany()
        const validCompanies = ctys.filter(val => val.name && val.name.trim() !== '');
        if (validCompanies.length > 0) {
            setListCty(validCompanies);
                ...
                let updatedColDefs = [
                    ...
                    },
                    {
                        ...
                        cellEditorParams: {
                            ...
                            filterList: true,
                            highlightMatch: true,
                            values: FIELD_VALUE_CAU_HINH.map((obj) => obj.field),
                    ...
                    },
                    {
                        ...
                        cellEditorParams: {
                            ...
                            highlightMatch: true,
                            values: params => {
                                let obj = FIELD_VALUE_CAU_HINH.find(e => e.field === params.data.field)
                                if (obj.values) {
                                    return obj.values
                                } else {
                                    return null
...

                    {
                        ...
                        cellEditorParams: {
                            ...
                            filterList: true,
                            highlightMatch: true,
                            values: listCty.map((cty) => cty.code),
    ...
    const handleAddRow = async () => {
        ...
        await createNewCauHinh(newData);
    ...
    const handleSaveData = async () => {
        ...
        setChangeBackgroud(!changeBackgroud)
    };
