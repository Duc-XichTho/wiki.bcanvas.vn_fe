...
import {createSDT2, getTkConArr} from "./logicSDT.js";
...
import {So_Tai_Khoan_DT} from "../../../../../Consts/TITLE_HEADER.js";
...
export default function SoTaiKhoanDT() {
    const headerTitle = So_Tai_Khoan_DT;
    const table = 'SoTaiKhoanDT';
    const tableCol = 'SoTaiKhoanDTCol';
    const tableFilter = 'SoTaiKhoanDTFilter';
    ...
    const initialSelectedTD2 = useRef(selectedTD2);
    ...
    const loadDataTaiKhoan = async (reset) => {
        ...
        let filteredData = data.filter(item => item.code == selectedTK);
    ...
    const loadData = async (reset = false) => {
        ...
        if (startMonth && endMonth && selectedTK) {
            let arrTK = getTkConArr(cdps, selectedTK)
            ...
            if (selectedTD) {
                ...
                listSTKDT = calSTKDT2(sktT1, JSON.parse(JSON.stringify(listDauKy1)), selectedTD, dt.fieldSKT, startMonth, endMonth, selectedTK);
                if (selectedTD2) {
                    let dt2 = LIST_TD_TKKT.find(item => item.field == selectedTD2);
                    let sktT2 = filteredData.filter(item => item[dt2.fieldSKT] && item[dt2.fieldSKT] !== '');
                    let listDauKy2 = listDauKy.filter(item => item[selectedTD] && item[selectedTD] !== '');
                    let listSTKDT2 = calSTKDT2(sktT2, JSON.parse(JSON.stringify(listDauKy2)), selectedTD2, dt2.fieldSKT, startMonth, endMonth, selectedTK);
    ...
    useEffect(() => {
        ...
        const isSelectedChanged = selectedTD !== initialSelectedTD.current || selectedTK !== initialSelectedTK.current || selectedTD2 !== initialSelectedTD2.current;
        if (isMonthChanged || isSelectedChanged) {
            setCheckLoc(true);
        }
    }, [startMonth, endMonth, selectedTD, selectedTK, selectedTD2]);
        ...
        <div className={css.headerPowersheet}>
            ...
            <div className={css.headerAction}>
                <div style={{display: "flex", alignItems: "center", gap: '5px', fontSize: '13px'}}>
                    <p>Tài khoản - </p> <TKKeToanSelect tkktList={tkktList} selectedTK={selectedTK}
                                                        setSelectedTK={setSelectedTK}/>
                </div>
                <div style={{display: "flex", alignItems: "center", gap: '5px', fontSize: '13px'}}>
                    <p>Đối tượng 1 - </p> <ListTDTKKTSelect tdList={tdList} selectedTD={selectedTD}
                                                            setSelectedTD={setSelectedTD}/>
                </div>
                <div style={{display: "flex", alignItems: "center", gap: '5px', fontSize: '13px'}}>
                    <p>Đối tượng 2 - </p> <ListTDTKKTSelect tdList={tdList} selectedTD={selectedTD2}
                                                            setSelectedTD={setSelectedTD2}/>
                </div>
