...
export function genDataM(dataSoKeToan, dataPBGV3, dataSettingPPTGT) {
    let newData = dataSoKeToan.map((item) => {
        ...
        return ({
            ...
            'CCPBLSX': genMoHinh(dataSettingPPTGT, item) === LSX ? TT : (existingItem ? existingItem.CCPBLSX : null),
...
export function mergeDataGV3M(rowData, dataPBGV2B, dataCCPB) {
    ...
    rowData.forEach(item => {
        if (item.CCPBLSX) {
            let ccpb = dataCCPB.find(cc => cc.name === item.CCPBLSX);
...
export function processDataGV3M(inputArray, listLSX) {
    let groupedData = {};
    inputArray.forEach(item => {
        ...
        if (!groupedData[key]) {
            groupedData[key] = {
                ...
                CCPBLSX: item.CCPBLSX,
                ky:item.ky,
            };
        }
        groupedData[key].so_tien_VND += so_tien;
        if (!groupedData[key][item.lenh_sx]) {
            groupedData[key][item.lenh_sx] = 0;
        }
        groupedData[key][item.lenh_sx] += so_tien;
    });
    return Object.values(groupedData).map(item => {
        item.so_tien_VND = item.so_tien_VND;
        Object.keys(item).forEach(key => {
            if (listLSX.some(e=> e.code === key)) {
                item[key] = item[key];
            }
        });
        return item;
