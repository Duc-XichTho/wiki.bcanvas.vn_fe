...
import {createDonHang, getDonHangByCode, getDonHangById, updateDonHang} from "../../../apis/donHangService";
...
const TaoDonHang2 = () => {
  ...
  const { idCard , option , idSelected } = useParams();
  ...
  const fetchDonHangByCode = async () => {
    try {
      const  data = await getDonHangById(idSelected);
  ...
  return (
    <>
      <Flex vertical  style={{padding: 10}}>
        <h3 style={{marginBottom: 10}} >{dataDonHangByCode?.code}</h3>
            ...
            <Flex vertical gap={5}>
              ...
              {dataDonHangByCode?.trang_thai === 'done'
                ? (
                  ...
                  >
                    <Card>
                      <Card.Grid style={{ ...gridStyle2, justifyContent: 'center' }}>
                        <Space>
