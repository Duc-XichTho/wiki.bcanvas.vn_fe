...
import { getAllTamUng, getTamUngDataById } from "../../../../../../apis/tamUngService.jsx";
...
export function PhieuGomTamUng({ isOpen, setIsOpen }) {
    ...
    async function fetchDHList() {
        ...
        const data = await getAllTamUng();
        const filtedData = data.filter(item => item.code);

        for (const item of filtedData) {
            const tamUngData = await getTamUngDataById(item.id);
            console.log(tamUngData);
            listData.push(tamUngData);
        }
            ...
            >
                <div style={{
                    ...
                    <div className={css.contentRight}>
                        {selected && <TamUngDetail phieu={selected} />}
                    </div>
                </div>
            </Modal>
            {isOpenModal &&
