...
import {getAllDonHang, getDonHangByCode} from "../../../../../../apis/donHangService.jsx";
...
import {getDetailData, getDetailDataGom} from "../../../../formCreate/GomPhieu/logicGom.js";
...
export function PhieuGom({isOpen, setIsOpen}) {
    ...
    useEffect(() => {
        ...
        if (selectedItem) {
            const updatedFields = selectedItem.fields.filter(item => item.field !== 'id' && item.field !== "donHang" && item.field !== "id_khach_hang");
        ...
        for (const dh of selectedDHList) {
            ...
            if (e && e.data && dataDetail.length !== 0) {
                dataDetail.forEach(detail => {
                    const data = {...detail, gia_xuat: detail.gia_xuat || 0, id_phieu_xuat: e.data.id, id: null}
                    tongTien += parseInt(data.gia_xuat * data.so_luong)
            ...
            await updateCardDetails(idCardNew, created_at, tongTien, dataNhanVien, genCode(CODE_PX, e.data.id, currentYear));
        //     setChainTemplate2Selected({
        //         type: 'chain2',
        //         data: {
    ...
    async function handleOpenModal() {
        ...
        for (const dh of selectedDHs) {
            let d = await getDonHangByCode(dh);
            listDH.push(d);
        }
                ...
                <div style={{
                    ...
                    <div className={css.bar}>
                        <div className={css.list}>
                            <span style={{marginLeft: '15px'}}>Danh sách đơn hàng</span>
                    ...
                    <div className={css.contentRight}>
                        {selected && <PhieuGomDetail phieu={selected}/>}
                    </div>
                </div>
            </Modal>
            {isOpenModal &&
