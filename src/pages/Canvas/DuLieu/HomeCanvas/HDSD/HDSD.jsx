import css from './HDSD.module.css';
...
export default function HDSD() {
	...
	const fetchNotes = async () => {
		try {
			...
			const filtered = data.filter(note => note.type == 'HDSD');
	...
	const handleClick = (note) => {
		navigate(`/canvas/${companySelect}/${buSelect}/home/guide/${note.id}`);
	...
	const handleDelete = async (id) => {
		try {
			...
			let newPath ;
			if (id == idNoteChart) {
				 newPath = `/canvas/${companySelect}/${buSelect}/home/guide`;
			} else {
				newPath = `/canvas/${companySelect}/${buSelect}/home/guide/${idNoteChart}`;
			}
			navigate(newPath);
				...
				<div className={css.topSection}>
					<div className={css.headerSidebar}>
						<div className={css.textGroup}>
							...
							>
								Bộ tài liệu hướng dẫn về cách sử dụng giải pháp B‑Canvas, cũng như các luồng công việc liên quan.
							</div>
						</div>
