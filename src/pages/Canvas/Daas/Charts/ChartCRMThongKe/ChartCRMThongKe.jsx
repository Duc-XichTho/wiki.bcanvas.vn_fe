...
export default function ChartCRMThongKe() {
    ...
    const loadChart = async () => {
        ...
        try {
            ...
            crmData.forEach(data => {
                ...
                const sale = data.ty_le_giam_gia ? parseFloat(data.ty_le_giam_gia) : 0;
                const weekData = chartData.find(item => item.month == data.week);
                if (weekData) {
                    weekData.thanh_tien += parseInt(data.thanh_tien);
                    weekData.giam_gia += data.thanh_tien * (1 - sale);
                } else {
                    const newWeekData = {
                        month: data.week,
                        thanh_tien: parseInt(data.thanh_tien),
                        giam_gia: data.thanh_tien * (1 - sale)
                    }
                    chartData.push(newWeekData);
                }
            });
            chartData.sort((a, b) => {
                const numA = parseInt(a.month.replace("W", ""), 10);
                const numB = parseInt(b.month.replace("W", ""), 10);
                return numA - numB;
            });
            const groups = ['thanh_tien', 'giam_gia']
            const interpolation = {
                type: 'smooth'
            };
            const series = prepareChartSeries(groups, chartData, interpolation);
            let areaChart = createSectionData('C1803 - Thống kê về sản phẩm bán', chartData, series, 'C1803 - Thống kê về sản phẩm bán')
