...
export default function ChartNhomSPLoiNhuan() {
    ...
    const key = "NHOMSP_LOINHUAN_CHART";
        ...
        try {
            ...
            for (let month = 1; month <= (currentMonthCanvas == 0 ? 12 : currentMonthCanvas); month++) {
                ...
                uniqueGroupsSP.forEach((unit) => {
                    let value = caculateSumRevenueWithMonthAndUnitSP(loi_nhuan_data_BCSP, mon, unit);
                    rowBCSP[unit?.toLowerCase()] = value == 0 ? NaN : value;
                });
            ...
            let loiNhuanSanPham = createSectionData('C1040 - Lợi nhuận nhóm SP', data_for_chart_line_BCSP, series_for_chart_line_bcsp, 'C1040 - Lợi nhuận nhóm SP');
            await setItemInIndexedDB2(key, data_for_chart_line_BCSP);
            setOptions(loiNhuanSanPham)
        } catch (error) {
            console.log(error);
