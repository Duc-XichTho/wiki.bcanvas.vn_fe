...
import {Cong_No_Phai_Thu, KHONG_THE_TRUY_CAP} from '../../../../../Consts/TITLE_HEADER';
// FUNCTION
...
export default function CongNoPhaiThu() {

    const headerTitle = Cong_No_Phai_Thu;
    const table = 'HoaDon';

    const tableCol = 'CONGNOPHAITHU';
    const tableFilter = 'CONGNOPHAITHU';
    const selectedTD = "TD_HoaDon";
    const selectedTD2 = "TD_HoaDon"
    const key = 'CONGNOPHAITHU';
    const tableStatusButton = 'CONGNOPHAITHUStatusButtonCanvas';
    ...
    useEffect(() => {  const fetchSettings = async () => {
    ...
    const loadData = async () => {
        if (companySelected &&  companySelected.length > 0) {
            ...
            hoaDonData = hoaDonData.filter(item => item.id_khach_hang && item.type == 'dau_vao');
