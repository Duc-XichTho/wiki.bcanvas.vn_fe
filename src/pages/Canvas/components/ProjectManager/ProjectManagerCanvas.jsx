...
import css from "./ProjectManagerCanvas.module.css";
import { MyContext } from "../../../../MyContext";
// COMPONENTS
import PJHeader from "../../../ProjectManager/PJHeader/PJHeader";
import LeftPanelCanvas from "./LeftPanel/LeftPanelCanvas";
import RightPanel from "../../../ProjectManager/RightPanel/RightPanel";
...
const ProjectManagerCanvas = () => {
  // const { projectId, stepId, taskId } = 1;
  const { companySelect, buSelect, projectId, stepId, taskId } = useParams();
  ...
  const handleProjectChange = (project) => {
    ...
    // navigate(`/canvas/${companySelect}/${buSelect}/cong-cu/project-manager/${project.id}`);
  ...
  const handleStepChange = (step) => {
    setSelectedProgressStep(step);
    // if (step) {
    //   navigate(`/canvas/${companySelect}/${buSelect}/cong-cu/project-manager/${selectedProject.id}/step/${step.id}`);
    // }
    setSelectedProgressTask(null);
  };

  const handleTaskChange = (task) => {
    if (task && task.id && selectedProgressStep) {
      setSelectedProgressTask(task);
      // navigate(
      //     `/canvas/${companySelect}/${buSelect}/cong-cu/project-manager/${selectedProject.id}/step/${selectedProgressStep.id}/task/${task.id}`,
      //     { replace: true }
      // );
    }
  };
...
export default ProjectManagerCanvas;
