...
import {BC_TIEN, BC_TON_KHO, Can_Doi_Phat_Sinh, KHONG_THE_TRUY_CAP} from "../../../Consts/TITLE_HEADER.js";
...
export default function BCTonKho2() {
    const { companySelect, id } = useParams();
    const headerTitle = BC_TON_KHO;
    const tableCol = 'BCTonKhoCol2';
    const tableFilter = 'BCTonKhoFilter2';
    const key = 'BAOCAO_TONKHO';
    ...
    const loadData = async () => {
        if (companySelected && companySelected.length > 0) {
            ...
            const KhoCCDC = calculateSumForCode(['153']);
            const KhoNVL = calculateSumForCode(['152']);
            const KhoHHTP = calculateSumForCode(['155', '156']);
            const KhoBTPDD = calculateSumForCode(['154']);

            let newData = [
                {name: 'Tổng tồn kho', net_cuoi_ky: KhoCCDC + KhoNVL + KhoHHTP + KhoBTPDD, layer: '1'},
                {name: 'Kho CCDC', net_cuoi_ky: KhoCCDC, layer: '1.1'},
                {name: 'Kho NVL', net_cuoi_ky: KhoNVL, layer: '1.2'},
                {name: 'Kho hàng hóa - thành phẩm', net_cuoi_ky: KhoHHTP, layer: '1.3'},
                {name: 'Kho bán thành phẩm- dở dang', net_cuoi_ky: KhoBTPDD, layer: '1.4'}
            ];
