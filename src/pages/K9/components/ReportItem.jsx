...
const ReportItem = ({
  ...
  const getDisplayTime = (item) => {
    // Try to get date from item.info.itemData["Ngày công bố"] first
    try {
      const info = typeof item.info === 'string' ? JSON.parse(item.info) : item.info;
      if (info?.itemData?.["Ngày công bố"]) {
        return getTimeAgo(info.itemData["Ngày công bố"]);
      }
    } catch (error) {
      console.error('Error parsing item.info:', error);
    }

    // Return null if no publication date found
    return null;
  };
  ...
  try {
    if (item.fileUrls) {
      fileUrls = typeof item.fileUrls === 'string' ? JSON.parse(item.fileUrls) : item.fileUrls;
    ...
    >
      <div className={styles.newsContent}>
        <div className={styles.newsTitle}>
          {item && item.info && item.info.itemData &&
              <div className={styles.iconReport}>
                <img src={'Solid-Blue.png'} alt="" />{item.info.itemData['Nguồn']}</div>}
          {title}
        </div>
        ...
        <div className={styles.newsExpandedContent}>
          ...
          <div className={styles.newsMeta}>
            ...
            {/* Hiển thị indicator có file đính kèm trong meta */}
            {fileUrls && Array.isArray(fileUrls) && fileUrls.length > 0 && (
              <span className={styles.fileIndicator} title={`${fileUrls.length} file đính kèm`}>
                <FileText  size={15} />
                {fileUrls.length}
              </span>
            )}
