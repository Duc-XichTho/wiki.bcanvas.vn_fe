...
import { createKtqtImportHistory, getAllKtqtImportHistory, hideAllKtqtImportHistoryByType } from '../../../../apisKTQT/ktqtImportHistoryService.jsx';
import './import.css';
...
const editableColumns = [
    ...
    { field: 'kmf', label: 'Khoản mục KQKD' },
    { field: 'project', label: 'Vụ việc' },
    { field: 'unit_code', label: 'Bộ phận' },
...
export default function KTQTImportContentGV({ onSuccess, phanLoaiDefault }) {
    ...
    const handleFetchHistory = async () => {
        try {
            const res = await getAllKtqtImportHistory();
            if (res && res.data) {
                setOldImportData(res.data.filter(item => item.import_type === phanLoai && item.show));
    ...
    // validate lại preview khi parsedData hoặc mappingValues thay đổi
    useEffect(() => {
