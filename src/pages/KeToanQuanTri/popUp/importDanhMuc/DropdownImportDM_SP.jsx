...
import {createNewProduct, updateProduct} from "../../../../apisKTQT/productService.jsx";

const DropdownImportDM_SP = ({table, columnDefs, company, data, title_table, reload, type_setting_group, listGroup, listUnit}) => {
    ...
    const handleFileUpload = (file) => {
        ...
        const exportColumns = columnDefs.filter(col =>
            !['action', 'id', 'code'].includes(col.field) &&
            !col.hide
        );
    ...
    const handleImport = async () => {
        try {
            ...
            const invalidUnitRecords = [];
            
            const validCompanyCodes = listCom.map(com => com.code?.toString().trim().toLowerCase());
            const validUnitCodes = listUnit.map(unit => unit.name?.toString().trim().toLowerCase());
            const newGroups = new Set();
            const newGroups1 = new Set();
            const newGroups2 = new Set();
            const newGroups3 = new Set();
            ...
            importedData.forEach(row => {
                ...
                Object.keys(row).forEach(key => {
                    ...
                    if (!['group', 'group1', 'group2', 'group3'].includes(key)) {
                ...
                const unitCode = processedRow.unit_code?.toString().trim().toLowerCase();

                // Validate company and unit
                ...
                if (!validUnitCodes.includes(unitCode)) {
                    invalidUnitRecords.push(`${processedRow.name || 'Không có tên'} (Đơn vị: ${processedRow.unit_code || 'Không có'})`);
                    return;
                }
                ...
                const hasAllFields = processedRow.name?.toString().trim() &&
                    ...
                    processedRow.unit_code?.toString().trim();
                ...
                // Check for existing record with matching name, company and groupType
                const existingRecord = existingRecords.find(existingRow =>
                    ...
                    existingRow.unit_code?.toString().trim().toUpperCase() === processedRow.unit_code
                );

                // Collect groups regardless of record status
                if (processedRow.group && !listGroup?.some(g => g?.groupType === 'group' && g.name?.toString().trim().toLowerCase() === processedRow.group.toLowerCase())) {
                    newGroups.add(processedRow.group);
                }
                if (processedRow.group1 && !listGroup?.some(g =>g?.groupType === 'group1' &&  g.name?.toString().trim().toLowerCase() === processedRow.group1.toLowerCase())) {
                    newGroups1.add(processedRow.group1);
                }
                if (processedRow.group2 && !listGroup?.some(g =>g?.groupType === 'group2' &&  g.name?.toString().trim().toLowerCase() === processedRow.group2.toLowerCase())) {
                    newGroups2.add(processedRow.group2);
                }
                if (processedRow.group3 && !listGroup?.some(g =>g?.groupType === 'group3' &&  g.name?.toString().trim().toLowerCase() === processedRow.group3.toLowerCase())) {
                    newGroups3.add(processedRow.group3);
                }

                processedRow.code = `${processedRow.name}-${processedRow.company}-${processedRow.unit_code}`;
            ...
            console.log(newGroups,newGroups1,newGroups2,newGroups3)
            // Create new groups first
            for (const newGroup of newGroups) {
                await createNewSettingGroup({
                    name: newGroup,
                    type: type_setting_group,
                    groupType: 'group'
                });
            }
            
            for (const newGroup of newGroups1) {
                await createNewSettingGroup({
                    name: newGroup,
                    type: type_setting_group,
                    groupType: 'group1'
            ...
            for (const newGroup of newGroups2) {
                await createNewSettingGroup({
                    name: newGroup,
                    type: type_setting_group,
                    groupType: 'group2'
            ...
            for (const newGroup of newGroups3) {
                await createNewSettingGroup({
                    name: newGroup,
                    type: type_setting_group,
                    groupType: 'group3'
            ...
            const summaryMessage = (<>
                ...
                <div>- {newGroups.size + newGroups1.size + newGroups2.size + newGroups3.size > 0 ? 
                    <>Đã tạo <span style={{color:'green'}}>{newGroups.size + newGroups1.size + newGroups2.size + newGroups3.size}</span> nhóm mới</> : 
                    <><span style={{color:'red'}}>0</span> nhóm được tạo mới</>}
                </div>
                ...
                <div>- <span style={{color:'red'}}>{invalidUnitRecords.length}</span> bản ghi bị bỏ qua do mã đơn vị không hợp lệ: {invalidUnitRecords.join(', ')}</div>
            </>);
...
export default DropdownImportDM_SP;