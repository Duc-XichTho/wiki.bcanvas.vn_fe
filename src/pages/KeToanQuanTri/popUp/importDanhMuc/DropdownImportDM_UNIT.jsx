...
import {createNewUnit, updateUnit} from "../../../../apisKTQT/unitService.jsx";

const DropdownImportDM_UNIT = ({table, columnDefs, company, data, title_table, reload, type_setting_group, listGroup, groupFieldName}) => {
    ...
    const handleImport = async () => {
        try {
            ...
            importedData.forEach(row => {
                ...
                // Check if company exists in listCom
                ...
                // Collect valid groups regardless of record status
                ...
                processedRow.code = `${processedRow.name}-${processedRow.company}`;
            ...
            const maxStt = listGroup?.reduce((max, group) => {
                const currentStt = parseInt(group.stt) || 0;
                return currentStt > max ? currentStt : max;
            }, 0);
            ...
            const summaryMessage = (<>
                ...
                <div>- {newGroups.size > 0 ? <>Đã tạo <span style={{color:'green'}}>{newGroups.size}</span> nhóm mới</> : <><span style={{color:'red'}}>0</span> nhóm được tạo mới</>}</div>
...
export default DropdownImportDM_UNIT;