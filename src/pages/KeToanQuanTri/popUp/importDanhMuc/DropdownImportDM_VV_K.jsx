...
const DropdownImportDM_VV_K = ({
    ...
    const handleImport = async () => {
        try {
            ...
            // Process and validate records first
            const processedRows = importedData.map(row => {
                const processedRow = {};
                ...
                // Check for duplicates and mark for update
                ...
                // Collect new groups regardless of record status
                if (processedRow[groupFieldName] && !listGroup?.some(g => g.name?.toString() === processedRow[groupFieldName])) {
                    newGroups.add(processedRow[groupFieldName]);
                }
            ...
            const summaryMessage = (<>
                ...
                <div>- {newGroups.size > 0 ? <>Đã tạo <span style={{color: 'green'}}>{newGroups.size}</span> nhóm
                    mới</> : <><span style={{color: 'red'}}>0</span> nhóm được tạo mới</>}</div>
                <div>- <span style={{color: 'red'}}>{invalidRecords.length}</span> bản ghi bị bỏ qua do thiếu thông
                    tin: {invalidRecords.join(', ')}</div>
                <div>- <span style={{color: 'red'}}>{invalidCompanyRecords.length}</span> bản ghi bị bỏ qua do mã công
                    ty không hợp lệ: {invalidCompanyRecords.join(', ')}</div>
                <div>- <span style={{color: 'red'}}>{invalidUnitRecords.length}</span> bản ghi bị bỏ qua do mã đơn vị
                    không hợp lệ: {invalidUnitRecords.join(', ')}</div>
            </>);
...
export default DropdownImportDM_VV_K;