...
export default function PhanBoKenh({company, call}) {
    const [editCount, setEditCount] = useState(0);
    const [isLoaded, setIsLoaded] = useState(false);
    const table = 'SoPhanBoKenh';
    const tableCol = 'SPBKenhhamCol';
    ...
    const tableStatusButton = 'SPBKenhStatusButton';
    ...
    async function loadData() {
        ...
        let listProduct = await getAllKenh();
        ...
        for (let i = 0; i < data.length; i++) {
            if (data[i].PBKENH) {
                let parsedPBSP = JSON.parse(data[i].PBKENH);
            ...
            if (data[i].kenh2 !== null && data[i].kenh2 !== '' && (!data[i].CCPBKENH || data[i].CCPBKENH === '')) {
                data[i]['pb' + data[i].kenh2] = +data[i].pl_value || 0;
                data[i].CCPBKENH = 'Trực tiếp';
            }
        }
        if (selectedButton) {
            if (selectedButton === 'Gián tiếp') {
                data = data.filter(item => item.CCPBKENH !== 'Tùy chỉnh' && item.CCPBKENH !== 'Trực tiếp');
            } else {
                data = data.filter(item => item.CCPBKENH === selectedButton);
            }
        }
        if (isSortByDay) {
            // Sắp xếp theo id
    ...
    useEffect(() => {
        loadData();
        setLoading(true);
        getAllKenh().then((data) => {
            setListProduct(data);
        });
        getAllCoCauPhanBo().then((data) => {
            data.push({name: 'Trực tiếp', type: 'Kenh', dp2: 1})
                ...
                let updatedColDefs = [
                    ...
                    },
                    {
                        ...
                        cellEditorParams: {
                            ...
                            filterList: true,
                            highlightMatch: true,
                            values: listCoChePhanBo.filter((e) => e.type === 'Kênh').map((p) => p.name),
                        },
                        ...filter()
                    },
                    {
    ...
    function updateCCPBSPData(event, ccpb) {
        ...
        event.data.PBKENH = JSON.stringify({teams: teamsArray});
    }
    ...
    const handleCellValueChanged = async (event) => {
        try {
            ...
            if (updateField === 'CCPBKENH') {
                if (event.newValue === 'Trực tiếp') {
                    event.data.PBKENH = null;
                } else {
            ...
            if (updateField.startsWith('pb')) {
                ...
                if (!event.data.PBKENH) {
                    event.data.PBKENH = '{"teams":[]}';
                }
                let teams = JSON.parse(event.data.PBKENH).teams;
                updateTeamValue(teams, unit, newValue);
                event.data.PBKENH = JSON.stringify({teams});
                event.data.CCPBKENH = 'Tùy chỉnh';
            }

            if (updateField === 'kenh') {
                if (event.data.company && event.data.kenh && event.data.kenh) {
                    event.data.kenh2 = `${event.data.kenh}-${event.data.company}-${event.data.unit_code}`;
                    event.data.PBKENH = null;
                    event.data.CCPBKENH = 'Trực tiếp';

                    let listProduct = await getAllKenh();
                    let isExistProduct = listProduct.some(e => e.code === event.data.kenh2);

                    if (!isExistProduct) {
                        await createNewKenh({
                            code: event.data.kenh2,
                            name: event.data.kenh,
                            dp: event.data.kenh,
        ...
        <>
            <div className={'header-powersheet'}>
                <div className={css.headerTitle}>
                    {!call && <>
                        <span>Phân Bổ Kênh<TooltipHeaderIcon table={table}/> </span>
