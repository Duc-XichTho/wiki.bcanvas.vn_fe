...
export default function PhanBoSanPham({company, call}) {
    const [editCount, setEditCount] = useState(0);
    const [isLoaded, setIsLoaded] = useState(false);
    const table = 'SoPhanBoSanPham';
    const tableCol = 'SPBSPhamCol';
    ...
    const tableStatusButton = 'SPBSanPhamStatusButton';
    ...
    async function loadData() {
        ...
        for (let i = 0; i < data.length; i++) {
            ...
            if (data[i].product2 !== null && data[i].product2 !== '' && (!data[i].CCBSP || data[i].CCBSP === '')) {
                data[i]['pb' + data[i].product2] = +data[i].pl_value || 0;
                data[i].CCBSP = 'Trực tiếp';
            }
        }
        if (selectedButton) {
            if (selectedButton === 'Gián tiếp') {
                data = data.filter(item => item.CCBSP !== 'Tùy chỉnh' && item.CCBSP !== 'Trực tiếp');
            } else {
                data = data.filter(item => item.CCBSP === selectedButton);
            }
        }
        if (isSortByDay) {
            // Sắp xếp theo id
    ...
    useEffect(() => {
        ...
        getAllCoCauPhanBo().then((data) => {
            data.push({name: 'Trực tiếp', type: 'Sản phẩm', dp2: 1})
                ...
                let updatedColDefs = [
                    ...
                    },
                    {
                        ...
                        cellEditorParams: {
                            ...
                            filterList: true,
                            highlightMatch: true,
                            values: listCoChePhanBo.filter((e) => e.type === 'Sản phẩm').map((p) => p.name),
                        },
                        ...filter()
                    },
                    {
    ...
    function updateCCPBSPData(event, ccpb) {
        ...
        event.data.PBSP = JSON.stringify({teams: teamsArray});
    }
    ...
    const handleCellValueChanged = async (event) => {
        try {
            ...
            if (updateField === 'CCBSP') {
                if (event.newValue === 'Trực tiếp') {
                    event.data.PBSP = null;
                } else {
            ...
            if (updateField.startsWith('pb')) {
                ...
                if (!event.data.PBSP) {
                    event.data.PBSP = '{"teams":[]}';
                }
                let teams = JSON.parse(event.data.PBSP).teams;
                updateTeamValue(teams, unit, newValue);
                event.data.PBSP = JSON.stringify({teams});
                event.data.CCBSP = 'Tùy chỉnh';
            }

            if (updateField === 'product') {
                if (event.data.company && event.data.unit_code && event.data.product) {
                    event.data.product2 = `${event.data.product}-${event.data.company}-${event.data.unit_code}`;
                    event.data.PBSP = null;
                    event.data.CCBSP = 'Trực tiếp';

                    let listProduct = await getAllProduct();
                    let isExistProduct = listProduct.some(e => e.code === event.data.product2);

                    if (!isExistProduct) {
                        await createNewProduct({
                            code: event.data.product2,
                            name: event.data.product,
                            dp: event.data.product,
            ...
            <div className={'header-powersheet'}>
                <div className={css.headerTitle}>
                    {!call && <>
                        ...
                        <span>Phân Bổ Sản Phẩm <TooltipHeaderIcon table={table}/> </span>
