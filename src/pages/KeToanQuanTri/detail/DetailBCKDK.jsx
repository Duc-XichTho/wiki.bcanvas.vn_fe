...
export default function DetailBCKDK({kmf, field, data, viewB, currentYear}) {
    let [groupK, month] = field.split('_')

    const table = 'DetailBCKDK';
    ...
    const fetchAllData = async () => {
        ...
        let kenhList = await getAllKenh()
        kenhList = kenhList.filter(e => e.group === groupK);
        ...
        const newArray = filteredStk.map(item => {
            if (item.CCPBKENH && item.PBKENH) {

                const parsedPBSP = JSON.parse(item.PBKENH);
            ...
            return [{
                ...item,
                kPB: item.kenh2,
                so_tien: item.pl_value,
                CCPBKENH: 'Trực tiếp'
            }];
        });
        let arr = newArray.flat().filter(e => e.so_tien && e.so_tien !== 0 && kenhList.some(sp => e.kPB === sp.code))
        if (viewB) {
