...
export default function DetailBCKDNhomVV({ kmf, field, data, viewB, currentYear, plType }) {
	let [group, month] = field.split('_');
	const table = 'DetailBCKDNhomVV';
	...
	const onGridReady = useCallback(async () => {
		...
		projectList = projectList.filter(e => e.group === group);
		...
		const newArray = filteredStk.map(item => {
			...
			if (item.CCPBPROJECT && item.PBPROJECT) {

				const parsedPBSP = JSON.parse(item.PBPROJECT);
			...
			return [{
				...
				CCPBPROJECT: 'Trực tiếp',
			}];
		});

		let arr = newArray.flat().filter(e => e.tienPB && e.tienPB !== 0 && projectList.some(sp => e.vvPB === sp.code));
		if (viewB) {
