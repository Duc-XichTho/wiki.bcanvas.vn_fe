...
import {getFirstThreeChars} from "../function/getTKForDetail.js";
import {getItemFromIndexedDB} from "../../../storage/storageService.js";

ModuleRegistry.registerModules([ClientSideRowModelModule, RowGroupingModule, SetFilterModule]);
export default function DetailCashReportTH({header, month, company}) {
    const table = 'DetailCashReportTH';
    ...
    const onGridReady = useCallback(async () => {
        setLoading(true)
        getItemFromIndexedDB('listSoKeToan').then(data => {
            const filteredData = data.filter(e => e.kmns === header && parseFloat(e.month) == month  && e.cash_value != 0 && e.cash_value !== null && e.consol?.toLowerCase() == 'consol')
    ...
    function calSum() {
        let sum = headerRenderer(rowData, `cash_value`)
