...
export default function DetailBCKDVV({kmf, field, currentYear}) {
    let [vv, month] = field.split('_')
    if (vv?.split(' | ')) {
        vv = vv?.split(' | ')[0]
    }
    const table = 'DetailBCKDVV';
    ...
    const fetchAllData = () => {
        ...
        getAllSoKeToan().then(data => {
            ...
            data = data.filter(e => e.consol?.toLowerCase() == 'consol' && e.pl_value !== '' && e.pl_value && e.year == currentYear && (e.month == month || month == 0))
                ...
                if (e.kmf == kmf) {
                    ...
                    if (e.project == vv) {
                        ...
                        if (e.PBPROJECT) {
                            ...
                            let pbdv = JSON.parse(e.PBPROJECT).teams;
                            return pbdv.some(pb => {

                                if (pb.team.split('-')[0] == vv) {
                                    e.so_tien = pb.tien;
                                    return true;
                                }
                                return false;
