...
import {AiButton, AiButton2, AllViewIcon, AllViewIcon2, FullnessIcon} from "../../../../../image/IconSVG.js";
...
import {
    calculateValue,
    calculateValueByKmf,
    calculateValueProduct,
    calculateValueTotal,
    calculateValueTotalYear
} from "../logic/logicActual.js";
...
export default function PlanActual({company}) {
   ...
   const [listUnits, setListUnits] = useState([]);
    ...
    const defaultColDef = useMemo(() => ({
        filter: true, cellStyle: {fontSize: '14.5px', color: 'var(--text-color)', fontFamily: 'var(--font-family)'}, resizeable: true, width: 20,
    }), []);
    ...
    async function loadData() {
        ...
        if (selectedUnit === 'Total') {
            ...
            rowData.forEach((item) => {
                ...
                for (let month = 0; month <= 12; month++) {
                    if (!item.layer.includes('.')) item['t0'] += parseFloat(item[`t${month}`] || 0);
                    if (!item.layer.includes('.')) item['t0_th'] += parseFloat(item[`t${month}_th`] || 0);
                }
            });
            ...
            rowData.forEach((item) => {
                ...
                for (let month = 1; month <= 12; month++) {
                    ...
                    if (!item.layer.includes('.') && item.header.includes('*')) item[`t${month}_th`] = totalTH;
                }
                for (let month = 0; month <= 12; month++) {
                    if (!item.layer.includes('.')) item['t0'] += parseFloat(item[`t${month}`] || 0);
                    if (!item.layer.includes('.')) item['t0_th'] += parseFloat(item[`t${month}_th`] || 0);
        ...
        setChartOption(
            createSectionData('Doanh thu', dataChartDT, [
                ...
                createSeries('name', 't0_ck', 'Cùng kỳ', 'bar'),
            ], 'Doanh Thu'))
        setChartOptionCF(
        ...
        <div className="collapsible-nav">
            <button className="collapsible-header" style={{cursor: "default"}}>
                <div className="navbar_action2">
                    <span className={'title-bc-14-10'}>Báo cáo Kế hoạch và thực hiện mục tiêu KQKD </span>
