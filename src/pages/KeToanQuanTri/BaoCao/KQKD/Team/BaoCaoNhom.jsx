...
export default function BaoCaoNhom() {
    const table = 'BaoCaoNhom';
    ...
    const getLocalStorageSettings = () => {
        ...
        return {
            ...
            selectedCompany: storedSettings?.selectedCompany ?? COMPANY_LIST[0].key,
        };
    };
    ...
    const [selectedCompany, setSelectedCompany] = useState(getLocalStorageSettings().selectedCompany);
    useEffect(() => {
        const tableSettings = {
            isShowView,
            isShowView2,
    ...
    }, [isShowView, isShowView2, isShowAll1, selectedMonth]);
    ...
    async function prepareData() {
        ...
        data = data.filter((e) => e.consol?.toLowerCase() == 'consol' && e.company === selectedCompany);
        let units = await getAllTeam();
        units = units.filter((e) => e.company === selectedCompany);
        ...
        let rowData = isShowView2
            ? calculateDataView2(data, units, kmfList, 'name', 'team_code')
            : calculateData(data, units, kmfList, 'name', 'team_code');
        if (isShowAll1) {
    ...
    useEffect(() => {
        prepareData();
    }, [selectedMonth, isShowAll1, selectedCompany]);
    useEffect(() => {
        prepareData();
    }, [isShowView, selectedCompany]);
            ...
            cellRenderer: (params) => {
                return (
                    <div className="cell-action-group">
                        <PopupCellActionBCKD {...params} field={field} allData={rowData} type={'NH'}
    ...
    const noteType = `KQKD-SP-T${selectedMonth}`; // Construct the type based on selectedMonth
    ...
    return (
        <>
            <div className="header-powersheet navbar_kqkd">
                <div className="navbar_title">
                    <div className="navbar_action">
                        <span className={'title-bc-14-10'}>Báo cáo KQKD Nhóm Theo Công Ty</span>
                        <span className={'title-bc-01-11'}><img src="/Group%20197.png" alt="Đơn vị: VND" style={{ width: '130px', marginLeft: '3px' }} /></span>
                    </div>
            ...
            >
                ...
                {isNoteVisible && ( // Conditionally render the NoteComponent
                    <NoteComponent
                        type={noteType} // Pass the constructed type
                    />
                )}
            </div>
        </>
