...
export default function BaoCaoPBNhomKenh2({ company }) {
    ...
    const table = 'BaoCaoPBNhomKenh2Canvas';
    const tableCol = 'BaoCaoPBNhomKenh2Col';
    const key = 'KQKD_NHOMK2';
    const { loadDataSoKeToan, currentYearKTQT, currentYearCanvas, currentMonthCanvas, userClasses, fetchUserClasses, uCSelected_CANVAS,} = useContext(MyContext);
    ...
    const tableStatusButton = 'BaoCaoPBNhomKenh2StatusButtonCanvas';
    ...
    useEffect(() => {
        ...
        saveSettings();
    }, [isShowView, isShowView2, isShowAll1, selectedUnit, isHideChart,isHideEmptyColumns, isShowInfo,]);
    ...
    async function prepareData(reload = false) {
      ...
      if (units?.length == 0 || !units) {
        ...
        let rowData = isShowView2
            ? await getItemFromIndexedDB2('BCNK2_2' + '_' + currentYearCanvas + '_' + companySelect)
            : await getItemFromIndexedDB2('BCNK2_1' + '_' + currentYearCanvas + '_' + companySelect);
        setLoading(true);
        ...
        if (isShowView2) {
            await setItemInIndexedDB2('BCNK2_2' + '_' + currentYearCanvas + '_' + companySelect, rowData);
        } else {
            await setItemInIndexedDB2('BCNK2_1' + '_' + currentYearCanvas + '_' + companySelect, rowData);
        }
        ...
        return {
            ...
            cellRenderer: (params) => {
                return (<div className="cell-action-group">
                    <PopupCellActionBCKD {...params} field={field} allData={rowData} type={'K'} view={isShowView2} currentYear={currentYearCanvas}/>
                </div>);
            }, ...Color(),
            cellStyle: (params) => {
