...
const AnalysisModal = ({
	...
	// Default prompt cho phân tích báo cáo KQKD
	const defaultPrompt = `Hãy phân tích báo cáo Kết quả kinh doanh (KQKD) với các thông tin sau:
- Năm: ${currentYearKTQT}
- Công ty: ${currentCompanyKTQT}
- Dữ liệu báo cáo: ${rowData.length} khoản mục

Vui lòng đưa ra:
1. Tổng quan về tình hình kinh doanh
2. Phân tích các khoản mục chính (doanh thu, chi phí, lợi nhuận)
3. Phân tích xu hướng theo tháng
4. Nhận xét và đánh giá hiệu quả hoạt động
5. Đề xuất các biện pháp cải thiện nếu cần
...
Hãy trình bày một cách chi tiết, dễ hiểu và có tính thực tiễn.`;
	...
	useEffect(() => {
		const loadAnalysisHistory = async () => {
			...
			try {
				// Tìm phân tích cho báo cáo KQKD tổng quát (không lọc theo user)
				...
				const existingAnalysis = allChatHistories.find(history => 
					...
					history.info?.baoCao === 'BaoCaoKDTongQuat' &&
					history.show === true
				);
	...
	const handleAnalyze = async () => {
		...
		setIsAnalyzing(true);
		try {
			// Chuẩn bị dữ liệu báo cáo đầy đủ (không lọc tháng)
			const reportData = {
				currentYearKTQT,
				currentCompanyKTQT,
				rowData: rowData,
				summary: {
					...
					revenueItems: rowData.filter(item => item.dp && item.dp.includes('Doanh Thu')).length,
					costItems: rowData.filter(item => item.dp && (item.dp.includes('CF') || item.dp.includes('Chi phí'))).length,
				}
			};
...
Hãy phân tích dữ liệu này một cách toàn diện và đưa ra nhận xét chi tiết.`;
			...
			const response = await aiGen2(
				fullPrompt,
				`Bạn là chuyên gia phân tích tài chính. Hãy phân tích báo cáo KQKD một cách chi tiết và chuyên nghiệp dựa trên dữ liệu được cung cấp.`,
				selectedAIModel,
				'text'
			);
	...
	const handleMouseMove = (e) => {
		...
		const maxX = window.innerWidth - 400; // 400px là width modal
		const maxY = window.innerHeight - 300; // 300px là height modal
			...
			>
				...
				>
					<div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
						<span style={{ fontWeight: '500' }}>Phân tích AI</span>
					</div>
					<Button
						type="text"
						size="small"
